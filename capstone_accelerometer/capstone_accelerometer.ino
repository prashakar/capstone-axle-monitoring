...
#define MPU6050_I2C_ADDRESS68 0x68
#define MPU6050_I2C_ADDRESS69 0x69
...
//  if (myFile) {
//    Serial.print("Writing to data.txt...");
//    //myFile.println("----NEXT DATA SET----");
//
//    myFile.println("---0x68 DATA---");
//
//    myFile.println(accel_t_gyro.value.x_accel);
//    //myFile.println(accel_t_gyro.value.y_accel);
//    //myFile.println(accel_t_gyro.value.z_accel);
//
//  // close the file:
//    myFile.close();
//    Serial.println("done.");
...
int MPU6050_read(int start, uint8_t *buffer, int size)
{
  ...
  Wire.beginTransmission(MPU6050_I2C_ADDRESS68);
  n = Wire.write(start);
  if (n != 1)
    return (-10);
  ...
  Wire.requestFrom(MPU6050_I2C_ADDRESS68, size, true);
  i = 0;
...
// There is no function for a single byte.
//
int MPU6050_read69(int start, uint8_t *buffer, int size)
{
  int i, n, error;

  Wire.beginTransmission(MPU6050_I2C_ADDRESS69);
  n = Wire.write(start);
  if (n != 1)
    return (-10);
  ...
  // Third parameter is true: relase I2C-bus after data is read.
  Wire.requestFrom(MPU6050_I2C_ADDRESS69, size, true);
  i = 0;
  while(Wire.available() && i<size)
  {
    buffer[i++]=Wire.read();
...
int MPU6050_write69(int start, const uint8_t *pData, int size)
{
  int n, error;
...
int MPU6050_write_reg69(int reg, uint8_t data)
{
  int error;

  error = MPU6050_write69(reg, &data, 1);
